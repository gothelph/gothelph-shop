...
//     const titleColumn = columns.has("name")
//       ? "name"
//       : columns.has("title")
//         ? "title"
//         : null;
...
export async function PATCH(req: Request) {
  ...
  if (!collectionId || !title) {
    return NextResponse.json(
      { error: "collectionId and title required" },
      { status: 400 },
    );
  }
    ...
    await pool.query(
      `UPDATE collections
       SET name = $1,
           description = $2
       WHERE id::text = $3`,
      [title, description || null, collectionId],
    );
...
// 🗑 УДАЛЕНИЕ КОЛЛЕКЦИИ (БЕЗ УБИЙСТВА ТОВАРОВ)
export async function DELETE(req: Request) {
  const auth = authenticateRequest(req, ["admin"]);
  if (!auth.ok) return auth.response;
  ...
  try {
    ...
    await pool.query(
      `DELETE FROM product_collections WHERE collection_id::text = $1`,
      [collectionId],
    );

    // потом саму коллекцию
    await pool.query(`DELETE FROM collections WHERE id::text = $1`, [
      collectionId,
    ]);
